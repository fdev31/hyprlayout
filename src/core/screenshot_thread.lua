...
if os.execute("which convert > /dev/null 2>&1") then
  to_rgba = function(src, dst, w, h)
    return os.execute(string.format('convert "%s" -resize "%dx%d" -depth 8 rgba:- > "%s" 2>/dev/null', src, w, h, dst))
  end
elseif os.execute("which magick > /dev/null 2>&1") then
  to_rgba = function(src, dst, w, h)
    return os.execute(string.format('magick "%s" -resize "%dx%d" -depth 8 rgba:- > "%s" 2>/dev/null', src, w, h, dst))
  end
elseif os.execute("which ffmpeg > /dev/null 2>&1") then
  to_rgba = function(src, dst, w, h)
    return os.execute(string.format('ffmpeg -y -i "%s" -vf "scale=%d:%d" -f rawvideo -pix_fmt rgba "%s" 2>/dev/null', src, w, h, dst))
  end
end

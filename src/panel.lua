...
function PANEL:layout(win_w, win_h)
	...
	self.title =
		Label.new(x, y, "General", { width = cw, height = 20, font_size = math.floor(15 * ui_scale), align = "center" })
	...
	self.shot_interval = Slider.new(x + label_w, y, cw - label_w, row_h, {
		...
		max = 10,
		step = 0.5,
		value = self.shot_interval_value or 10,
		scale = ui_scale,
		on_change = function(val)
			self:on_shot_interval_change(val)
		end,
	})
	y = y + row_h + 5
	...
	local function track(w)
		if w then
			table.insert(self._scroll_widgets, { widget = w, base_y = w.rect.y })
		end
	end
	...
	if content_h <= interior_h then
		...
		self.status = Label.new(
			...
			content_bottom,
			"",
			{ width = cw, height = status_h, font_size = math.floor(14 * ui_scale), color = { 1, 0.9, 0.5 } }
		)
